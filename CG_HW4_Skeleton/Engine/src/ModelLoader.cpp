...
namespace Engine {
    ModelLoader::ModelLoader(std::string path) {
        ...
        const aiScene *scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace);
    ...
    void ModelLoader::GenerateMesh(Mesh *_mesh, unsigned int meshIdx) {
        ...
        printf("has material: %d\n", scene->mNumMaterials);

        _mesh->AddAttribute(4);    // position
        _mesh->AddAttribute(4); // normal
        if (hasUV) {
            _mesh->AddAttribute(2); // uv coordinate
            _mesh->AddAttribute(4); // Tangent
        }
        for (size_t i = 0; i < mesh->mNumVertices; i++) {
            ...
            if (hasUV) {
                _mesh->AddVertexData(mesh->mTextureCoords[0][i].x);
                _mesh->AddVertexData(mesh->mTextureCoords[0][i].y);
                _mesh->AddVertexData(mesh->mTangents[i].x);
                _mesh->AddVertexData(mesh->mTangents[i].y);
                _mesh->AddVertexData(mesh->mTangents[i].z);
