...
void MyMaterial::CreateMaterial() {
    ...
    _program->AttachShader("./Resources/Materials/MyFragmentShader.glsl", GL_FRAGMENT_SHADER);
    _program->LinkShader();
...
void MyMaterial::UpdateAmbientReflectance(glm::vec3 color) {
    glUseProgram(_program->GetProgramId());
    GLuint location = glGetUniformLocation(_program->GetProgramId(), "ambient_reflectance");
    glUniform3fv(location, 1, (float *) &color);
}

void MyMaterial::UpdateDiffuseReflectance(glm::vec3 color) {
    glUseProgram(_program->GetProgramId());
    GLuint location = glGetUniformLocation(_program->GetProgramId(), "diffuse_reflectance");
    glUniform3fv(location, 1, (float *) &color);
}

void MyMaterial::UpdateSpecularReflectance(glm::vec3 color) {
    glUseProgram(_program->GetProgramId());
    GLuint location = glGetUniformLocation(_program->GetProgramId(), "specular_reflectance");
    glUniform3fv(location, 1, (float *) &color);
}

void MyMaterial::UpdateCameraPosition(glm::vec3 position) {
    glUseProgram(_program->GetProgramId());
...
void MyMaterial::UpdateLight(std::vector<Light> &lights) {
